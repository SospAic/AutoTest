...
class OpDraw(DrawOp):
    ...
    def Do(self, dc, xoffset, yoffset):
        if self._op == DRAWOP_DRAW_LINE:
            ...
            dc.DrawArc(self._x2 + xoffset, self._y2 + yoffset, self._x3 + xoffset, self._y3 + yoffset,
                       self._x1 + xoffset, self._y1 + yoffset)
        elif self._op == DRAWOP_DRAW_ELLIPTIC_ARC:
            dc.DrawEllipticArc(self._x1 + xoffset, self._y1 + yoffset, self._x2, self._y2,
                               self._x3 * 360 / (2 * math.pi), self._y3 * 360 / (2 * math.pi))
    ...
    def Rotate(self, x, y, theta, sinTheta, cosTheta):
        ...
        if self._op == DRAWOP_DRAW_LINE:
            ...
            newBottomRightX = oldBottomRightX * cosTheta - oldBottomRightY * sinTheta + x * (
                        1 - cosTheta) + y * sinTheta
            newBottomRightY = oldBottomRightX * sinTheta + oldBottomRightY * cosTheta + y * (
                        1 - cosTheta) + x * sinTheta
...
class PseudoMetaFile(object):
    ...
    def GetBounds(self):
        ...
        for op in self._ops:
            if op.GetOp() in [DRAWOP_DRAW_LINE, DRAWOP_DRAW_RECT, DRAWOP_DRAW_ROUNDED_RECT, DRAWOP_DRAW_ELLIPSE,
                              DRAWOP_DRAW_POINT, DRAWOP_DRAW_TEXT]:
                if op._x1 < minX:
                    minX = op._x1
                if op._x1 > maxX:
                    maxX = op._x1
