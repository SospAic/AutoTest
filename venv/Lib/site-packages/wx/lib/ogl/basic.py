...
class Shape(ShapeEvtHandler):
    ...
    def FormatText(self, dc, s, i=0):
        ...
        CentreText(dc, region.GetFormattedText(), self._xpos, self._ypos, actualW - 2 * self._textMarginX,
                   actualH - 2 * self._textMarginY, region.GetFormatMode())
    ...
    def Recentre(self, dc):
        ...
        for region in self._regions:
            CentreText(dc, region.GetFormattedText(), self._xpos, self._ypos, w - 2 * self._textMarginX,
    ...
    def OnDrawContents(self, dc):
        ...
        for region in self._regions:
            ...
            if not self._formatted:
                CentreText(dc, region.GetFormattedText(), self._xpos, self._ypos, bound_x - 2 * self._textMarginX,
                           bound_y - 2 * self._textMarginY, region.GetFormatMode())
                self._formatted = True

            if not self.GetDisableLabel():
                DrawFormattedText(dc, region.GetFormattedText(), self._xpos, self._ypos,
                                  bound_x - 2 * self._textMarginX, bound_y - 2 * self._textMarginY,
                                  region.GetFormatMode())
    ...
    def EraseLinks(self, dc, attachment=-1, recurse=False):
        ...
        for line in self._lines:
            if attachment == -1 or (
                    line.GetTo() == self and line.GetAttachmentTo() == attachment or line.GetFrom() == self and line.GetAttachmentFrom() == attachment):
                line.GetEventHandler().OnErase(dc)
    ...
    def MoveLineToNewAttachment(self, dc, to_move, x, y):
        ...
        for line in newOrdering:
            if line.GetTo() == self and oldAttachment == line.GetAttachmentTo() or \
                ...
                if self.AttachmentSortTest(newAttachment, newPoint, thisPoint) and self.AttachmentSortTest(
                        newAttachment, lastPoint, newPoint):
                    found = True
    ...
    def OnSizingDragLeft(self, pt, draw, x, y, keys=0, attachment=0):
        ...
        if self.GetCentreResize():
            ...
            self.GetEventHandler().OnDrawOutline(dc, pt._controlPointDragPosX, pt._controlPointDragPosY, newWidth,
                                                 newHeight)
    ...
    def OnDraw(self, dc):
        ...
        if self._shadowMode != SHADOW_NONE:
            ...
            if self._cornerRadius:
                dc.DrawRoundedRectangle(x1 + self._shadowOffsetX, y1 + self._shadowOffsetY, self._width, self._height,
                                        self._cornerRadius)
            else:
...
class PolygonShape(Shape):
    ...
    def SetSize(self, new_width, new_height, recursive=True):
        ...
        for i in range(max(len(self._points), len(self._originalPoints))):
            self._points[i] = wx.Point(self._originalPoints[i][0] * x_proportion,
                                       self._originalPoints[i][1] * y_proportion)
    ...
    def Rotate(self, x, y, theta):
        ...
        for i in range(len(self._points)):
            ...
            self._points[i] = x1 * cosTheta - y1 * sinTheta + x * (
                        1 - cosTheta) + y * sinTheta, x1 * sinTheta + y1 * cosTheta + y * (1 - cosTheta) + x * sinTheta

        for i in range(len(self._originalPoints)):
            x1, y1 = self._originalPoints[i]

            self._originalPoints[i] = x1 * cosTheta - y1 * sinTheta + x * (
                        1 - cosTheta) + y * sinTheta, x1 * sinTheta + y1 * cosTheta + y * (1 - cosTheta) + x * sinTheta
...
class EllipseShape(Shape):
    ...
    def OnDraw(self, dc):
        ...
        if self._brush:
            dc.SetBrush(self._brush)
        dc.DrawEllipse(self._xpos - self.GetWidth() / 2.0, self._ypos - self.GetHeight() / 2.0, self.GetWidth(),
                       self.GetHeight())
    ...
    def GetAttachmentPosition(self, attachment, nth=0, no_arcs=1, line=None):
        ...
        if self._attachmentMode != ATTACHMENT_MODE_NONE:
            ...
            if physicalAttachment == 0:
                ...
                return DrawArcToEllipse(self._xpos, self._ypos, self._width, self._height, x,
                                        self._ypos - self._height - 500, x, self._ypos)
            elif physicalAttachment == 1:
                x = right
                if self._spaceAttachments:
                    ...
                    y = self._ypos
                return DrawArcToEllipse(self._xpos, self._ypos, self._width, self._height,
                                        self._xpos + self._width + 500, y, self._xpos, y)
            elif physicalAttachment == 2:
                if self._spaceAttachments:
                    x = left + (nth + 1) * self._width / (no_arcs + 1.0)
                else:
...
class PolygonControlPoint(ControlPoint):
    ...
    def CalculateNewSize(self, x, y):
        ...
        dist = math.sqrt(
            (x - self._shape.GetX()) * (x - self._shape.GetX()) + (y - self._shape.GetY()) * (y - self._shape.GetY()))
