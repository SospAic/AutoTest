...
class LineShape(Shape):
    ...
    def HitTest(self, x, y):
        ...
        for i in range(len(self._lineControlPoints) - 1):
            ...
            if abs(
                    distance_from_seg) < extra and distance_from_prev >= 0 and distance_from_prev <= seg_len or inLabelRegion:
    ...
    def OnMoveLink(self, dc, moveControlPoints=True):
        ...
        if self._from == self._to and self._from.GetAttachmentMode() != ATTACHMENT_MODE_NONE and moveControlPoints and self._lineControlPoints and not (
    ...
    def FindLineEndPoints(self):
        ...
        if len(self._lineControlPoints) > 2 and self._initialised:
            if self._from.GetAttachmentMode() != ATTACHMENT_MODE_NONE:
                ...
                end_x, end_y = self._from.GetPerimeterPoint(self._from.GetX(), self._from.GetY(), second_point[0],
                                                            second_point[1])
            ...
            if self._to.GetAttachmentMode() != ATTACHMENT_MODE_NONE:
                ...
                other_end_x, other_end_y = self._to.GetPerimeterPoint(self._to.GetX(), self._to.GetY(),
                                                                      second_last_point[0], second_last_point[1])
        else:
    ...
    def MakeControlPoints(self):
        ...
        if self._canvas and self._lineControlPoints:
            ...
            control = LineControlPoint(self._canvas, self, CONTROL_POINT_SIZE, first[0], first[1],
            ...
            for point in self._lineControlPoints[1:-1]:
                control = LineControlPoint(self._canvas, self, CONTROL_POINT_SIZE, point[0], point[1],
                                           CONTROL_POINT_LINE)
                control._point = point
                self._canvas.AddShape(control)
                self._controlPoints.append(control)

            control = LineControlPoint(self._canvas, self, CONTROL_POINT_SIZE, last[0], last[1],
