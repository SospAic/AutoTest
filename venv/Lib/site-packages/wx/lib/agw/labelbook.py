        ...
        for i in range(len(self._pagesInfoVec)):
            ...
            if not style & INB_SHOW_ONLY_TEXT and self._pagesInfoVec[i].GetImageIndex() != -1:
                ...
                if bUseYcoord:
                    ...
                    imgYcoord = (style & INB_SHOW_ONLY_IMAGES and [pos + self._nImgSize / 2] or [pos + imgTopPadding])[
                        0]
...
class LabelContainer(ImageContainerBase):
    ...
    def OnPaint(self, event):
        ...
        if self.HasAGWFlag(INB_GRADIENT_BACKGROUND) and not self._skin.IsOk():
            ...
            ArtManager.Get().PaintStraightGradientBox(dc, wx.Rect(0, 0, size.x / 2, size.y), startColour, endColour,
                                                      False)
            ArtManager.Get().PaintStraightGradientBox(dc, wx.Rect(size.x / 2, 0, size.x / 2, size.y), endColour,
                                                      startColour, False)
    ...
    def InitializeColours(self):
        ...
        self._coloursMap.update(
            {INB_TAB_AREA_BACKGROUND_COLOUR: ArtManager.Get().LightColour(ArtManager.Get().FrameColour(), 50)})
