...
class RibbonPanel(RibbonControl):
    ...
    def HasExtButton(self):
        ...
        return (self._flags & RIBBON_PANEL_EXT_BUTTON) and (
                    bar.GetAGWWindowStyleFlag() & RIBBON_BAR_SHOW_PANEL_EXT_BUTTONS)
    ...
    def Realize(self):
        ...
        if self._art != None:
            temp_dc = wx.ClientDC(self)
            self._smallest_unminimised_size = self._art.GetPanelSize(temp_dc, self, wx.Size(*minimum_children_size),
                                                                     None)
    ...
    def ShowExpanded(self):
        ...
        pos = self.GetExpandedPosition(wx.Rect(self.GetScreenPosition(), self.GetSize()), size,
        ...
        self._expanded_panel = RibbonPanel(container, wx.ID_ANY, self.GetLabel(), self._minimised_icon, wx.Point(0, 0),
                                           size, self._flags)
