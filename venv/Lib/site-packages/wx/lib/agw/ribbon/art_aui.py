...
class RibbonAUIArtProvider(RibbonMSWArtProvider):
    ...
    def DrawTabCtrlBackground(self, dc, wnd, rect):
        ...
        dc.GradientFillLinear(gradient_rect, self._tab_ctrl_background_colour,
                              self._tab_ctrl_background_gradient_colour, wx.SOUTH)
    ...
    def DrawTab(self, dc, wnd, tab):
        ...
        if tab.active or tab.hovered:
            ...
            dc.GradientFillLinear(grad_rect, self._tab_active_background_colour,
            ...
            dc.GradientFillLinear(grad_rect, self._tab_hover_background_top_colour,
    ...
    def DrawPanelBackground(self, dc, wnd, rect):
        ...
        dc.DrawLine(label_rect.x, label_rect.y + label_rect.height, label_rect.x + label_rect.width,
        ...
        if wnd.HasExtButton():
            if wnd.IsExtButtonHovered():
                ...
                dc.DrawBitmap(self._panel_extension_bitmap[1], label_rect.GetRight() - 10, label_rect.GetBottom() - 10,
                              True)

            else:
                dc.DrawBitmap(self._panel_extension_bitmap[0], label_rect.GetRight() - 10, label_rect.GetBottom() - 10,
                              True)
    ...
    def DrawPartialPanelBackground(self, dc, wnd, rect):
        ...
        if not paint_rect.IsEmpty():
            starting_colour = RibbonInterpolateColour(bg_clr, bg_grad_clr, paint_rect.y, background.y,
                                                      background.y + background.height)
            ending_colour = RibbonInterpolateColour(bg_clr, bg_grad_clr, paint_rect.y + paint_rect.height, background.y,
                                                    background.y + background.height)
            paint_rect.x -= offset.x
            paint_rect.y -= offset.y
    ...
    def DrawGalleryButton(self, dc, rect, state, bitmaps):
        ...
        if state == RIBBON_GALLERY_BUTTON_NORMAL:
            dc.GradientFillLinear(reduced_rect, self._gallery_button_background_colour,
                                  self._gallery_button_background_gradient_colour, wx.SOUTH)
    ...
    def DrawTool(self, dc, wnd, rect, bitmap, kind, state):
        ...
        if kind & RIBBON_BUTTON_DROPDOWN:
            ...
            dc.DrawBitmap(self._toolbar_drop_bitmap, bg_rect.x + avail_width + 2, bg_rect.y + (bg_rect.height / 2) - 2,
                          True)
