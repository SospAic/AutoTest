...
class RibbonPage(RibbonControl):
    ...
    def Layout(self):
        ...
        origin_ = wx.Point(self._art.GetMetric(RIBBON_ART_PAGE_BORDER_LEFT_SIZE),
                           self._art.GetMetric(RIBBON_ART_PAGE_BORDER_TOP_SIZE))
        ...
        if major_axis == wx.HORIZONTAL:
            ...
            minor_axis_size = self.GetSize().GetHeight() - origin.y - self._art.GetMetric(
                RIBBON_ART_PAGE_BORDER_BOTTOM_SIZE)
        else:
            gap = self._art.GetMetric(RIBBON_ART_PANEL_Y_SEPARATION_SIZE)
            minor_axis_size = self.GetSize().GetWidth() - origin.x - self._art.GetMetric(
        ...
        for iteration in range(1, 3):
            ...
            if iteration == 1:
                if major_axis == wx.HORIZONTAL:
                    available_space = self._size_in_major_axis_for_children - self._art.GetMetric(
                        RIBBON_ART_PAGE_BORDER_RIGHT_SIZE) - origin.x + gap
                else:
                    available_space = self._size_in_major_axis_for_children - self._art.GetMetric(
                        RIBBON_ART_PAGE_BORDER_BOTTOM_SIZE) - origin.y + gap
    ...
    def GetMinSize(self):
        ...
        if self.GetMajorAxis() == wx.HORIZONTAL:
            minSize.x = -1
            if minSize.y != -1:
                minSize.y += self._art.GetMetric(RIBBON_ART_PAGE_BORDER_TOP_SIZE) + self._art.GetMetric(
                    RIBBON_ART_PAGE_BORDER_BOTTOM_SIZE)

        else:
            if minSize.x != -1:
                minSize.x += self._art.GetMetric(RIBBON_ART_PAGE_BORDER_LEFT_SIZE) + self._art.GetMetric(
                    RIBBON_ART_PAGE_BORDER_RIGHT_SIZE)
    ...
    def DoGetBestSize(self):
        ...
        if best.x != -1:
            best.x += self._art.GetMetric(RIBBON_ART_PAGE_BORDER_LEFT_SIZE) + self._art.GetMetric(
                RIBBON_ART_PAGE_BORDER_RIGHT_SIZE)

        if best.y != -1:
            best.y += self._art.GetMetric(RIBBON_ART_PAGE_BORDER_TOP_SIZE) + self._art.GetMetric(
                RIBBON_ART_PAGE_BORDER_BOTTOM_SIZE)
