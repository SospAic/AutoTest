...
class UltimateListHeaderData(object):
    ...
    def IsHit(self, x, y):
        ...
        return ((x >= self._xpos) and (x <= self._xpos + self._width) and (y >= self._ypos) and (
                    y <= self._ypos + self._height))
...
class UltimateListLineData(object):
    ...
    def SetPosition(self, x, y, spacing):
        ...
        if mode in [ULC_ICON, ULC_SMALL_ICON]:
            ...
            if item.HasImage():
                self._gi._rectIcon.x = self._gi._rectAll.x + 4 + (
    ...
    def DrawInReportMode(self, dc, line, rect, rectHL, highlighted, current, enabled, oldPN, oldBR):
        ...
        for col, item in enumerate(self._items):
            ...
            if overflow:
                itemRect = wx.Rect(xOld - 2 * HEADER_OFFSET_X, rect.y, rectHL.width - xSize - HEADER_OFFSET_X,
                                   rect.height)
            ...
            if item.HasText():
                ...
                itemRect = wx.Rect(itemRect.x + MARGIN_BETWEEN_TEXT_AND_ICON, itemRect.y, itemRect.width - 8,
                                   itemRect.height)
    ...
    def OnPaint(self, event):
        ...
        for i in range(numColumns):
            ...
            if kind in [1, 2]:
                ...
                self._owner.DrawCheckbox(dc, x + HEADER_OFFSET_X, HEADER_OFFSET_Y + (h - 4 - iy) / 2, kind, checked,
                                         enabled)
...
class UltimateListMainWindow(wx.ScrolledWindow):
    ...
    def ResetLineDimensions(self, force=False):
        ...
        if (self.HasAGWFlag(ULC_REPORT) and self.HasAGWFlag(
                ULC_HAS_VARIABLE_ROW_HEIGHT) and not self.IsVirtual()) or force:
    ...
    def GetLineLabelRect(self, line, col=0):
        ...
        rect = wx.Rect(image_x + HEADER_OFFSET_X, self.GetLineY(line), self.GetColumnWidth(col) - image_width,
    ...
    def HighlightLine(self, line, highlight=True):
        ...
        if changed and not dontNotify:
            self.SendNotify(line,
                            (highlight and [wxEVT_COMMAND_LIST_ITEM_SELECTED] or [wxEVT_COMMAND_LIST_ITEM_DESELECTED])[
                                0])
    ...
    def OnMouse(self, event):
        ...
        if event.GetEventType() == wx.wxEVT_MOTION and not event.Dragging():
            ...
            if current >= 0 and current < count and self.HasAGWFlag(
                    ULC_TRACK_SELECT) and not self._hoverTimer.IsRunning():
                self._hoverItem = current
                self._hoverTimer.Start(HOVER_TIME, wx.TIMER_ONE_SHOT)

            if newItem and newItem.IsHyperText() and (
                    hitResult & ULC_HITTEST_ONITEMLABEL) and theItem and theItem.IsEnabled():
        ...
        if event.Dragging():
            ...
            if not self._isDragging:
                ...
                command = (event.RightIsDown() and [wxEVT_COMMAND_LIST_BEGIN_RDRAG] or [wxEVT_COMMAND_LIST_BEGIN_DRAG])[
                    0]
                le = UltimateListEvent(command, self.GetParent().GetId())
                le.SetEventObject(self.GetParent())
        ...
        if event.LeftUp():
            ...
            if self._lastOnSame:
                if (current == self._current) and (hitResult == ULC_HITTEST_ONITEMLABEL) and self.HasAGWFlag(
                        ULC_EDIT_LABELS):
        ...
        if event.RightDown():
            ...
            if self.HasAGWFlag(ULC_STICKY_HIGHLIGHT) and self.HasAGWFlag(ULC_STICKY_NOSELEVENT) and self.HasAGWFlag(
                    ULC_SEND_LEFTCLICK):
    ...
    def OnChar(self, event):
        ...
        if keyCode == wx.WXK_UP:
            ...
            steps = (self.InReportView() and [self._linesPerPage - 1] or [
                self._linesPerPage - (self._current % self._linesPerPage) - 1])[0]
            index = self._current + steps
            count = self.GetItemCount()
