...
class AuiDefaultDockArt(object):
    ...
    **Button Identifiers**
    ...
    def DrawIcon(self, dc, rect, pane):
        ...
        if pane.icon.IsOk():
            if pane.HasCaptionLeft():
                ...
                dc.DrawBitmap(bmp.ConvertToBitmap(), rect.x + (rect.width - pane.icon.GetWidth()) // 2,
    ...
    def DrawCaption(self, dc, window, text, rect, pane):
        ...
        if captionLeft:
            dc.DrawRotatedText(draw_text, rect.x + (rect.width / 2) - (h / 2) - 1,
...
class ModernDockArt(AuiDefaultDockArt):
    ...
    def DrawCaption(self, dc, window, text, rect, pane):
        ...
        if captionLeft:
            dc.DrawRotatedText(draw_text, rect.x + (rect.width / 2) - (h / 2) - diff,
                               rect.y + rect.height - 3 - caption_offset, 90)
        else:
    ...
    def DrawCaptionBackground(self, dc, rect, pane):
        ...
        if self.usingTheme:
            ...
            if active:
                winxptheme.DrawThemeBackground(self.hTheme1, int(dc.GetHandle()), 5, 1,
                                               (rc.top, rc.left, rc.right, rc.bottom), None)
            else:
                winxptheme.DrawThemeBackground(self.hTheme1, int(dc.GetHandle()), 5, 2,
                                               (rc.top, rc.left, rc.right, rc.bottom), None)
    ...
    def DrawPaneButton(self, dc, window, button, button_state, rect, pane):
        ...
        if self.usingTheme:
            ...
            try:
                winxptheme.DrawThemeBackground(hTheme, int(dc.GetHandle()), btntype, state,
                                               (rc.top, rc.left, rc.right, rc.bottom), None)
            except TypeError:
                return
