...
class Document(wx.EvtHandler):
    ...
    def Save(self):
        ...
        if not self.IsDocumentModificationDateCorrect():
            ...
            res = wx.MessageBox(_("'%s' has been modified outside of %s.  Overwrite '%s' with current changes?") % (
    ...
    def SaveAs(self):
        ...
        descr = docTemplate.GetDescription() + _(" (") + docTemplate.GetFileFilter() + _(
            ") |") + docTemplate.GetFileFilter()  # spacing is important, make sure there is no space after the "|", it causes a bug on wx_gtk
        ...
        try:
            ...
            if os.path.exists(filename):
                ...
                if not os.access(filename, os.W_OK):
                    wx.MessageBox(
                        "Could not save '%s'.  No write permission to overwrite existing file." % FileNameFromPath(
                            filename),
                        msgTitle,
...
class DocManager(wx.EvtHandler):
    ...
    def OnPreview(self, event):
        ...
        if printout:
            ...
            frame = wx.PreviewFrame(preview, mimicFrame, _("Print Preview"), mimicFrame.GetPosition(),
                                    mimicFrame.GetSize())
            ...
            for document in self._docs:
                if document.GetFilename() and os.path.normcase(document.GetFilename()) == os.path.normcase(path):
                    ...
                    if not document.IsDocumentModificationDateCorrect():
                        ...
                        if res == wx.YES:
                            if not self.CloseDocument(document, False):
                                wx.MessageBox(
                                    _("Couldn't reload '%s'.  Unable to close current '%s'.") % (shortName, shortName))
                                return None
                    ...
                    if firstView and firstView.GetFrame() and not (flags & DOC_NO_VIEW):
                        ...
                        if hasattr(firstView.GetFrame(),
                                   "IsIconized") and firstView.GetFrame().IsIconized():  # Not in wxWindows code but useful nonetheless
        ...
        if wx.Platform == "__WXMSW__" or wx.Platform == "__WXGTK__" or wx.Platform == "__WXMAC__":
            ...
            for temp in templates:
                if temp.IsVisible():
                    if len(descr) > 0:
                        descr = descr + _('|')
                    descr = descr + temp.GetDescription() + _(" (") + temp.GetFileFilter() + _(
                        ") |") + temp.GetFileFilter()  # spacing is important, make sure there is no space after the "|", it causes a bug on wx_gtk
            descr = _(
                "All|*.*|%s") % descr  # spacing is important, make sure there is no space after the "|", it causes a bug on wx_gtk
        else:
            descr = _("*.*")
...
class DocMDIParentFrame(wx.MDIParentFrame):
    ...
    def __init__(self, manager, frame, id, title, pos=wx.DefaultPosition, size=wx.DefaultSize,
                 style=wx.DEFAULT_FRAME_STYLE, name="frame"):
        """
        Constructor.
        """
