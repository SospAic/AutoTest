...
class Field:
    ...
    def _ValidateParameters(self, **kwargs):
        ...
        if 'validRegex' in kwargs:  # (set/changed)
            if self._validRegex:
                try:
                    ...
                    ##                    dbg(indent=0, suspend=0)
                    raise TypeError(
                        '%s: validRegex "%s" not a legal regular expression' % (str(self._index), self._validRegex))
            else:
                self._filter = None
        ...
        if 'validRange' in kwargs:  # (set/changed)
            ...
            self._rangeLow = 0
            if self._validRange:
                if not isinstance(self._validRange, tuple) or len(self._validRange) != 2 or self._validRange[0] > \
                        self._validRange[1]:
                    ##                    dbg(indent=0, suspend=0)
                ...
                if hasattr(self, '_template'):
                    ...
                    for choice in self._choices:
                        ...
                        if not self.IsValid(choice):
                            ##                            dbg(indent=0, suspend=0)
                            ve = ValueError('%s: "%s" is not a valid value for the control as specified.' % (
...
class MaskedEditMixin:
    ...
    def __init__(self, name='MaskedEdit', **kwargs):
        ...
        # Validate legitimate set of parameters:
        for key in kwargs.keys():
            if key.replace('Color', 'Colour') not in list(MaskedEditMixin.valid_ctrl_params.keys()) + list(
                    Field.valid_params.keys()):
    ...
    def _calcTemplate(self, reset_fillchar, reset_default):
        ...
        if default_set:
            ...
            if not self.IsEmpty(self._defaultValue) and not self.IsValid(self._defaultValue):
                ####                dbg(indent=0)
                ve = ValueError(
                    'Default value of "%s" is not a valid value for control "%s"' % (self._defaultValue, self.name))
            ...
            if field._choices:
                ...
                for choice in field._choices:
                    ...
                    if not valid_paste:
                        ####                        dbg(indent=0)
                        ve = ValueError(
                            '"%s" could not be entered into field %d of control "%s"' % (choice, index, self.name))
                        ve.value = choice
                        ve.index = index
                        raise ve
                    elif replace_to > end:
        ...
        if self._isFloat or self._isInt:
            if self.controlInitialized:
                ...
                if len(value) < len(self._ctrl_constraints._mask):
                    newvalue = value
                    if self._useParens and len(newvalue) < len(self._ctrl_constraints._mask) and newvalue.find(
                            '(') == -1:
                        newvalue += ' '
            ...
            if wx.WXK_UP not in self._keyhandlers:
                self._SetKeycodeHandler(wx.WXK_UP,
                                        self._OnUpNumeric)  # (adds "shift" to up arrow, and calls _OnChangeField)
            ...
            if ord(self._decimalChar) not in self._keyhandlers or self._keyhandlers[
                ord(self._decimalChar)] != self._OnDecimalPoint:
                self._SetKeyHandler(self._decimalChar, self._OnDecimalPoint)

            if ord(self._shiftDecimalChar) not in self._keyhandlers or self._keyhandlers[
                ord(self._shiftDecimalChar)] != self._OnChangeField:
            ...
            if ord(self._fields[0]._groupChar) not in self._keyhandlers or self._keyhandlers[
                ord(self._fields[0]._groupChar)] != self._OnGroupChar:
    ...
    def _OnArrow(self, event):
        ...
        if keycode in (wx.WXK_RIGHT, wx.WXK_DOWN, wx.WXK_NUMPAD_RIGHT, wx.WXK_NUMPAD_DOWN):
            if ((not self._isTemplateChar(pos) and pos + 1 > entry_end)
                ...
                self._AdjustField(pos)
        elif keycode in (wx.WXK_LEFT, wx.WXK_UP, wx.WXK_NUMPAD_LEFT,
                         wx.WXK_NUMPAD_UP) and sel_start == sel_to and pos > 0 and self._isTemplateChar(pos - 1):
            ##            dbg('adjusting field')
            self._AdjustField(pos)
    ...
    def _OnErase(self, event=None, just_return_value=False):
        ...
        if ((sel_to == 0 and key == wx.WXK_BACK)
                ...
                or (sel_to == self._masklength and sel_start == sel_to and key in (
                wx.WXK_DELETE, wx.WXK_NUMPAD_DELETE) and not field._insertRight)
                or (self._signOk and self._useParens
                    and sel_start == sel_to
                    and sel_to == self._masklength - 1
                    and value[sel_to] == ' ' and key in (
                    wx.WXK_DELETE, wx.WXK_NUMPAD_DELETE) and not field._insertRight)):
            if not wx.Validator.IsSilent():
                wx.Bell()
            ##            dbg(indent=0)
        ...
        if (field._insertRight  # an insert-right field
                ...
                and ((sel_to == sel_start  # and no selection
                      ...
                      and key in (
                      wx.WXK_BACK, wx.WXK_DELETE, wx.WXK_NUMPAD_DELETE))  # and either delete or backspace key
                     or  # or
                ...
                if sel_start == sel_to:
                    ...
                    if field._padZero and not value[start:sel_to].replace('0', '').replace(' ', '').replace(
                        ...
                        newchar = self._template[
                            sel_to]  ## get an original template character to "clear" the current char
            ...
            if field != field_end or sel_to >= end_of_input:
                ...
                if sel_to == edit_end and field._index < self._field_indices[-1]:
                    edit_start, edit_end = self._FindFieldExtent(
                        self._findNextEntry(edit_end))  # go to end of next field:
                    end = edit_end
        ...
        if chr(key) in ('-', '+', '(', ')') or (chr(key) == " " and pos == self._signpos):
            ...
            if self._isNeg and self._signpos is not None and self._signpos != -1:
                if self._useParens and self._right_signpos is not None:
                    text = text[:self._signpos] + '(' + text[self._signpos + 1:self._right_signpos] + ')' + text[
                                                                                                            self._right_signpos + 1:]
                else:
    ...
    def _OnAutoCompleteField(self, event):
        ...
        if field._hasList:
            ...
            if keycode in (wx.WXK_PAGEUP, wx.WXK_UP, wx.WXK_NUMPAD_PAGEUP, wx.WXK_NUMPAD_UP):
                ...
                direction = 1
            match_index, partial_match = self._autoComplete(direction, choices, text,
                                                            compareNoCase=field._compareNoCase,
                                                            current_index=field._autoCompleteIndex)
            if (match_index is None
                    and (keycode in self._autoCompleteKeycodes + [wx.WXK_PAGEUP, wx.WXK_PAGEDOWN, wx.WXK_NUMPAD_PAGEUP,
                                                                  wx.WXK_NUMPAD_PAGEDOWN]
                         or (keycode in [wx.WXK_UP, wx.WXK_DOWN, wx.WXK_NUMPAD_UP,
                                         wx.WXK_NUMPAD_DOWN] and event.ShiftDown()))):
        ...
        if self._isMaskChar(pos):
            ...
            if approved and checkRegex:
                ...
                if right_insert:
                    ...
                    at = pos
                if allowAutoSelect:
                    newvalue, ignore, ignore, ignore, ignore = self._insertKey(char, at, sel_start, sel_to, value,
    ...
    def _getAbsValue(self, candidate=None):
        ...
        if self._isInt:
            ...
            try:
                groupchar = self._fields[0]._groupChar
                value = float(text.replace(groupchar, '').replace(self._decimalChar, '.').replace('(', '-').replace(')',
                                                                                                                    '').replace(
                    ' ', ''))
            ##                dbg('value:', value)
            except:
            ...
            if field._autoSelect:
                ...
                if match_index is not None and partial_match:
                    ...
                    if self._ctrl_constraints._insertRight:
                        ...
                        newpos = self._masklength - (len(self._ctrl_constraints._choices[match_index].strip()) - len(
                            matched_str.strip()))
    ...
    def _validateNumeric(self, candidate=None):
        ...
        try:
            ...
            if self._isFloat:
                number = float(
                    value.replace(groupchar, '').replace(self._decimalChar, '.').replace('(', '-').replace(')', ''))
            else:
    ...
    def _fixSelection(self):
        ...
        if (sel_start == 0 and sel_to >= len(
                self._mask)  # (can be greater in numeric controls because of reserved space)
    ...
    def _validatePaste(self, paste_text, sel_start, sel_to, raise_on_invalid=False):
        ...
        while valid_paste and i < length_considered and replace_to < self._masklength:
            ...
            if not self._isTemplateChar(replace_to) and self._isCharAllowed(char, replace_to, allowAutoSelect=False,
                                                                            ignoreInsertRight=True):
    ...
    def _Paste(self, value=None, raise_on_invalid=False, just_return_value=False):
        ...
        if paste_text is not None:
            ...
            edit_start, edit_end = field._extent
            new_pos = None
            if field._allowInsert and sel_to <= edit_end and (
                    sel_start + len(paste_text) < edit_end or field._insertRight):
                if field._insertRight:
            ...
            if signed:
                ...
                if new_text:
                    if self._useParens:
                        new_text = new_text[:signpos] + '(' + new_text[signpos + 1:right_signpos] + ')' + new_text[
                                                                                                          right_signpos + 1:]
                    else:
...
Try entering nonsensical or partial values in validated fields to see what happens (use ctrl-s to test the valid status)."""
        ...
        controls = [
            ...
            ("Date (MDY) + Time\n(with default)", "##/##/#### ##:## AM", 'BCDEFGHIJKLMNOQRSTUVWXYZ', 'DFR!', "", (), [],
