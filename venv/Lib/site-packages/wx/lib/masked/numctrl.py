...
class NumCtrl(BaseMaskedTextCtrl, NumCtrlAccessorsMixin):
        ...
        if (('integerWidth' in kwargs and kwargs['integerWidth'] != self._integerWidth)
            ...
            if 'fractionWidth' in kwargs:
                if not isinstance(kwargs['fractionWidth'], int):
                    raise AttributeError(
                        'invalid fractionWidth (%s) specified; expected integer' % repr(kwargs['fractionWidth']))
    ...
    def _GetNumValue(self, value):
        ...
        """
        return value.replace(self._groupChar, '').replace(self._decimalChar, '.').replace('(', '-').replace(')',
                                                                                                            '').strip()
    ...
    def _SetValue(self, value):
        ...
        if replacement is not None:
            ...
            sel_start = BaseMaskedTextCtrl.GetValue(self).find(
                str(abs(replacement)))  # find where it put the 1, so we can select it
    ...
    def _Undo(self, value=None, prev=None):
        ...
        ignore, (new_sel_start, new_sel_to) = BaseMaskedTextCtrl._Undo(self, adjvalue, adjprev,
