...
class SlicesShell(editwindow.EditWindow):
    ...
    def ToggleShellMode(self, enableShellMode=None):
        if enableShellMode == None:
            if self.mode == 'ShellMode':
                self.mode = 'SlicesMode'
            elif self.mode == 'SlicesMode':
                self.mode = 'ShellMode'
    ...
    def IsAllowedPair(self, m1, m2):
        ...
        if (m1 & i_s) and (m1 & g_s):  # 1
            if (m2 & i_s) and (m2 & g_s):
                return True  # 1
            elif (m2 & i_m) and (m2 & g_m):
                return True  # 2
            elif (m2 & i_e) and (m2 & g_m):
                return True  # 3
            elif (m2 & i_e) and (m2 & g_e):
                return True  # 4
            elif (m2 & o_s) and (m2 & g_s):
                return False  # 5
            elif (m2 & o_s) and (m2 & g_m):
                return True  # 6
            elif (m2 & o_s) and (m2 & g_e):
                return True  # 7
            elif (m2 & o_m) and (m2 & g_m):
                return False  # 8
            elif (m2 & o_e) and (m2 & g_e):
                return False  # 9
            else:
                return False
        elif (m1 & i_m) and (m1 & g_m):  # 2
            if (m2 & i_m) and (m2 & g_m):
                return True  # 2
            elif (m2 & i_e) and (m2 & g_m):
                return True  # 3
            elif (m2 & i_e) and (m2 & g_e):
    ...
    def LoadPySlicesFile(self, fid):
        ...
        line = fid.readline()
        if line == inputStartText:
            ioStartTypes.append('input');removeComment = False
        elif line == outputStartText:
            ioStartTypes.append('output');removeComment = True
        else:
            print(invalidFileString); return
        ...
        for i in fid:
            if i == groupingStartText:
                ...
                if removeComment:
                    w = i[1:].replace(os.linesep, '\n')
                else:
                    w = i.replace(os.linesep, '\n')
