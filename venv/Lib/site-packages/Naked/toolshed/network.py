...
class HTTP():
    ...
    def get(self, follow_redirects=True):
        try:
            ...
            if DEBUG_FLAG:
                sys.stderr.write(
                    "Naked Framework Error: Unable to perform GET request with the URL " + self.url + " using the get() method (Naked.toolshed.network.py)")
            raise e
    ...
    def get_bin(self):
        try:
            ...
            if DEBUG_FLAG:
                sys.stderr.write(
                    "Naked Framework Error: Unable to perform GET request with the URL " + self.url + " using the get_data() method (Naked.toolshed.network.py)")
            raise e
    ...
    def get_bin_write_file(self, filepath="", suppress_output=False, overwrite_existing=False):
        try:
            ...
            if (filepath == "" and len(self.url) > 1):
                filepath = self.url.split('/')[
                    -1]  # use the filename from URL and working directory as default if not specified
            if not suppress_output:
            ...
            if DEBUG_FLAG:
                sys.stderr.write(
                    "Naked Framework Error: Unable to perform GET request and write file with the URL " + self.url + " using the get_bin_write_file() method (Naked.toolshed.network.py)")
            raise e
    ...
    def get_txt_write_file(self, filepath="", suppress_output=False, overwrite_existing=False):
        try:
            ...
            if DEBUG_FLAG:
                sys.stderr.write(
                    "Naked Framework Error: Unable to perform GET request and write file with the URL " + self.url + " using the get_data_write_txt() method (Naked.toolshed.network.py)")
            raise e
    ...
    def head(self):
        try:
            ...
            if DEBUG_FLAG:
                sys.stderr.write(
                    "Naked Framework Error: Unable to perform a HEAD request with the head() method (Naked.toolshed.network.py).")
            raise e
    ...
    def post(self, follow_redirects=True):
        try:
            ...
            if DEBUG_FLAG:
                sys.stderr.exit(
                    "Naked Framework Error: Unable to perform a POST request with the post() method (Naked.toolshed.network.py).")
            raise e
    ...
    def post_bin(self):
        try:
            ...
            if DEBUG_FLAG:
                sys.stderr.exit(
                    "Naked Framework Error: Unable to perform POST request with the post_bin() method (Naked.toolshed.network.py).")
            raise e
    ...
    def post_bin_write_file(self, filepath="", suppress_output=False, overwrite_existing=False):
        try:
            ...
            if not suppress_output:
                sys.stdout.write(
                    "Downloading file from " + self.url + "...")  # provide information about the download to user
                sys.stdout.flush()
            ...
            if DEBUG_FLAG:
                sys.stderr.write(
                    "Naked Framework Error: Unable to perform POST request and write file with the URL " + self.url + " using the post_data_write_bin() method (Naked.toolshed.network.py)")
            raise e
    ...
    def response(self):
        try:
            ...
            if DEBUG_FLAG:
                sys.stderr.write(
                    "Naked Framework Error: Unable to return the response from your HTTP request with the response() method (Naked.toolshed.network.py).")
            raise e
    ...
    def get_status_ok(self):
        try:
            ...
            if DEBUG_FLAG:
                sys.stderr.write(
                    "Naked Framework Error: Unable to obtain the HTTP status with the get_status_ok() method (Naked.toolshed.network.py).")
            raise e
    ...
    def post_status_ok(self):
        try:
            ...
            if DEBUG_FLAG:
                sys.stderr.write(
                    "Naked Framework Error: Unable to obtain the HTTP status with the post_status_ok() method (Naked.toolshed.network.py).")
            raise e
