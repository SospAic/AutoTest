...
class Template(str):
    def __new__(cls, template_text, open_delimiter="{{", close_delimiter="}}", escape_regex=False):
        ...
        obj.varlist = obj._make_var_list(template_text,
                                         escape_regex)  #contains all unique parsed variables from the template in a list
        return obj
    ...
    def _make_var_list(self, template_text, escape_regex=False):
        ...
        var_list = re.findall(match_pat,
                              template_text)  #generate a list that contains the capture group from the matches (i.e. the variables in the template)
        return set(
            var_list)  # remove duplicate entries by converting to set (and lookup speed improvement from hashing)
