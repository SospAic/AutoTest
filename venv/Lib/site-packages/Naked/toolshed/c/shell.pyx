...
def execute(command):
    try:
        ...
        if DEBUG_FLAG:
            sys.stderr.write(
                "Naked Framework Error: unable to run the shell command with the execute() function (Naked.toolshed.shell.py).")
        raise e
...
def run(command, suppress_stdout=False, suppress_stderr=False, suppress_exit_status_call=True):
    try:
        ...
        if DEBUG_FLAG:
            sys.stderr.write(
                "Naked Framework Error: unable to run the shell command with the run() function (Naked.toolshed.shell.py).")
        raise e
...
def execute_rb(file_path, arguments=""):
    try:
        ...
        if DEBUG_FLAG:
            sys.stderr.write(
                "Naked Framework Error: unable to run the shell command with the run_rb() function (Naked.toolshed.shell.py).")
        raise e
...
def run_rb(file_path, arguments="", suppress_stdout=False, suppress_stderr=False, suppress_exit_status_call=True):
    try:
        if len(arguments) > 0:
            ...
            rb_command = 'ruby ' + file_path
        return run(rb_command, suppress_stdout, suppress_stderr,
                   suppress_exit_status_call)  # return result of run() of the ruby file
    except Exception as e:
        if DEBUG_FLAG:
            sys.stderr.write(
                "Naked Framework Error: unable to run the shell command with the run_rb() function (Naked.toolshed.shell.py).")
...
def muterun_rb(file_path, arguments=""):
    try:
        ...
        if DEBUG_FLAG:
            sys.stderr.write(
                "Naked Framework Error: unable to run the shell command with the muterun_rb() function (Naked.toolshed.shell.py).")
        raise e
...
def execute_js(file_path, arguments=""):
    try:
        ...
        if DEBUG_FLAG:
            sys.stderr.write(
                "Naked Framework Error: unable to run the shell command with the run_js() function (Naked.toolshed.shell.py).")
        raise e
#------------------------------------------------------------------------------
...
def run_js(file_path, arguments="", suppress_stdout=False, suppress_stderr=False, suppress_exit_status_call=True):
    try:
        if len(arguments) > 0:
            ...
            js_command = 'node ' + file_path
        return run(js_command, suppress_stdout, suppress_stderr,
...
def muterun_js(file_path, arguments=""):
    try:
        ...
        if DEBUG_FLAG:
            sys.stderr.write(
                "Naked Framework Error: unable to run the shell command with the muterun_js() function (Naked.toolshed.shell.py).")
        raise e
...
class Environment():
    ...
    def is_var(self, var_name):
        try:
            ...
            if DEBUG_FLAG:
                sys.stderr.write(
                    "Naked Framework Error: unable to determine if the variable is included in the shell variable list with the is_var() method (Naked.toolshed.shell).")
            raise e
    ...
    def get_var(self, var_name):
        try:
            ...
            if DEBUG_FLAG:
                sys.stderr.write(
                    "Naked Framework Error: unable to return the requested shell variable with the get_var() method (Naked.toolshed.shell).")
            raise e
    ...
    def get_split_var_list(self, var_name):
        try:
            ...
            if DEBUG_FLAG:
                sys.stderr.write(
                    "Naked Framework Error: unable to return environment variable list with the get_split_var_list() method (Naked.toolshed.shell).")
            raise e
