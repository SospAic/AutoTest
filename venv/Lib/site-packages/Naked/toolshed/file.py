...
class FileWriter(IO):
    ...
    def append(self, text):
        try:
            ...
            if not file_exists(
                    self.filepath):  # confirm that file exists, if not raise IOError (assuming that developer expected existing file if using append)
    ...
    def gzip(self, text, compression_level=6):
        try:
            ...
            if DEBUG_FLAG:
                sys.stderr.write(
                    "Naked Framework Error: unable to gzip compress the file with the gzip method (Naked.toolshed.file.py).")
            raise e
    ...
    def write(self, text):
        try:
            ...
            if DEBUG_FLAG:
                sys.stderr.write(
                    "Naked Framework Error: Unable to write to requested file with the write() method (Naked.toolshed.file.py).")
            raise e
    ...
    def write_as(self, text, the_encoding=""):
        try:
            if the_encoding == "":  # if the developer did not include the encoding type, raise an exception
                raise RuntimeError(
                    "The text encoding was not specified as an argument to the write_as() method (Naked.toolshed.file.py:write_as).")
            import codecs
            ...
            if DEBUG_FLAG:
                sys.stderr.write(
                    "Naked Framework Error: unable to write file with the specified encoding using the write_as() method (Naked.toolshed.file.py).")
            raise e
    ...
    def write_bin(self, binary_data):
        try:
            ...
            if DEBUG_FLAG:
                sys.stderr.write(
                    "Naked Framework Error: Unable to write binary data to file with the write_bin method (Naked.toolshed.file.py).")
            raise e
    ...
    def safe_write(self, text):
        ...
        if not os.path.exists(self.filepath):  # if the file does not exist, then can write
            try:
                ...
                if DEBUG_FLAG:
                    sys.stderr.write(
                        "Naked Framework Error: Unable to write to requested file with the safe_write() method (Naked.toolshed.file.py).")
                raise e
        else:
    ...
    def safe_write_bin(self, file_data):
        try:
            ...
            if DEBUG_FLAG:
                sys.stderr.write(
                    "Naked Framework Error: Unable to write to requested file with the safe_write_bin() method (Naked.toolshed.file.py).")
            raise e
    ...
    def write_utf8(self, text):
        try:
            ...
            if DEBUG_FLAG:
                sys.stderr.write(
                    "Naked Framework Error: Unable to open file for write with the write_utf8() method (Naked.toolshed.file.py).")
            raise ioe
        try:
            import unicodedata
            norm_text = unicodedata.normalize('NFKD', text)  # NKFD normalization of the unicode data before write
            ...
            if DEBUG_FLAG:
                sys.stderr.write(
                    "Naked Framework Error: Unable to write UTF-8 encoded text to file with the write_utf8() method (Naked.toolshed.file.py).")
            raise e
        finally:
            f.close()
...
class FileReader(IO):
    ...
    def read(self):
        try:
            ...
            if DEBUG_FLAG:
                sys.stderr.write(
                    "Naked Framework Error: Unable to read text from the requested file with the read() method (Naked.toolshed.file.py).")
            raise e
    ...
    def read_bin(self):
        try:
            ...
            if DEBUG_FLAG:
                sys.stderr.write(
                    "Naked Framework Error: Unable to read the binary data from the file with the read_bin method (Naked.toolshed.file.py).")
            raise e
    ...
    def read_as(self, the_encoding):
        try:
            if the_encoding == "":
                raise RuntimeError(
                    "The text file encoding was not specified as an argument to the read_as method (Naked.toolshed.file.py:read_as).")
            import codecs
            ...
            if DEBUG_FLAG:
                sys.stderr.write(
                    "Naked Framework Error: Unable to read the file with the developer specified text encoding with the read_as method (Naked.toolshed.file.py).")
            raise e
    ...
    def readlines(self):
        try:
            ...
            if DEBUG_FLAG:
                sys.stderr.write(
                    "Naked Framework Error: Unable to read text from the requested file with the readlines() method (Naked.toolshed.file.py).")
            raise e
    ...
    def readlines_as(self, dev_spec_encoding):
        try:
            if dev_spec_encoding == "":
                raise RuntimeError(
                    "The text file encoding was not specified as an argument to the readlines_as method (Naked.toolshed.file.py:readlines_as).")
            import codecs
            ...
            if DEBUG_FLAG:
                sys.stderr.write(
                    "Naked Framework Error: unable to read lines in the specified encoding with the readlines_as method (Naked.toolshed.file.py).")
            raise e
    ...
    def readlines_utf8(self):
        try:
            ...
            if DEBUG_FLAG:
                sys.stderr.write(
                    "Naked Framework Error: unable to read lines in the unicode file with the readlines_utf8 method (Naked.toolshed.file.py)")
            raise e
        ...
        try:
            ...
            with gzip.open(self.filepath, 'rb') as gzip_reader:
                ...
                if encoding in ["utf-8", "utf8", "utf_8", "UTF-8", "UTF8", "UTF_8"]:
                    ...
                    import unicodedata
                    norm_data = unicodedata.normalize('NFKD',
                                                      file_data)  # NKFD normalization of the unicode data before passing back to the caller
                    return norm_data
                else:
                    return file_data
        except Exception as e:
            if DEBUG_FLAG:
                sys.stderr.write(
                    "Naked Framework Error: Unable to read from the gzip compressed file with the read_gzip() method (Naked.toolshed.file.py).")
            raise e
    ...
    def read_utf8(self):
        try:
            ...
            if DEBUG_FLAG:
                sys.stderr.write(
                    "Naked Framework Error: Unable to open file for read with read_utf8() method (Naked.toolshed.file.py).")
            raise ioe
        try:
            textstring = f.read()
            import unicodedata
            norm_text = unicodedata.normalize('NFKD',
                                              textstring)  # NKFD normalization of the unicode data before returns
            return norm_text
        except Exception as e:
            if DEBUG_FLAG:
                sys.stderr.write(
                    "Naked Framework Error: Unable to read the file with UTF-8 encoding using the read_utf8() method (Naked.toolshed.file.py).")
            raise e
        finally:
            f.close()
