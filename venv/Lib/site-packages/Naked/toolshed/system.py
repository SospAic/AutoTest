...
def filename(filepath):
    try:
        ...
        if DEBUG_FLAG:
            sys.stderr.write(
                "Naked Framework Error: unable to return base filename from filename() function (Naked.toolshed.system).")
        raise e
...
def file_extension(filepath):
    try:
        ...
        if DEBUG_FLAG:
            sys.stderr.write(
                "Naked Framework Error: unable to return file extension with file_extension() function (Naked.toolshed.system).")
        raise e
...
def directory(filepath):
    try:
        ...
        if DEBUG_FLAG:
            sys.stderr.write(
                "Naked Framework Error: unable to return directory path to file with directory() function (Naked.toolshed.system).")
        raise e
...
def make_path(*path_list):
    try:
        ...
        if DEBUG_FLAG:
            sys.stderr.write(
                "Naked Framework Error: unable to make OS independent path with the make_path() function (Naked.toolshed.system).")
        raise e
...
def currentdir_to_basefile(func):
    try:
        ...
        def wrapper(file_name, *args, **kwargs):
            ...
            return func(full_path, *args,
                        **kwargs)  # return the original function with the full path to file as first argument

        return wrapper
    except Exception as e:
        if DEBUG_FLAG:
            sys.stderr.write(
                "Naked Framework Error: error with the currentdir_to_basefile() decorator function (Naked.toolshed.system).")
        raise e
...
def currentdir_firstparam(func):
    try:
        ...
        if DEBUG_FLAG:
            sys.stderr.write(
                "Naked Framework Error: error with the currentdir_firstargument() decorator function (Naked.toolshed.system).")
        raise e
...
def currentdir_lastparam(func):
    try:
        ...
        if DEBUG_FLAG:
            sys.stderr.write(
                "Naked Framework Error: error with the currentdir_lastargument() decorator function (Naked.toolshed.system).")
        raise e
...
def fullpath(file_name):
    try:
        ...
        if DEBUG_FLAG:
            sys.stderr.write(
                "Naked Framework Error: unable to return absolute path to the file with the fullpath() function (Naked.toolshed.system).")
        raise e
...
def cwd(dir=""):
    try:
        ...
        if DEBUG_FLAG:
            sys.stderr.write(
                "Naked Framework Error: unable to return the current working directory with the cwd() function (Naked.toolshed.system).")
        raise e
...
def make_dirs(dirpath):
    try:
        ...
        if ose.errno != errno.EEXIST:  # directory already exists
            if DEBUG_FLAG:
                sys.stderr.write(
                    "Naked Framework Error: Could not write the directory path passed as an argument to the make_dirs() function (Naked.toolshed.system).")
            raise ose
        else:
            return False
    except Exception as e:
...
def file_exists(filepath):
    try:
        ...
        if DEBUG_FLAG:
            sys.stderr.write(
                "Naked Framework Error: error with test for the presence of the file with the file_exists() method (Naked.toolshed.system).")
        raise e
...
def is_file(filepath):
    try:
        ...
        if DEBUG_FLAG:
            sys.stderr.write(
                "Naked Framework Error: error with test for file with the is_file() function (Naked.toolshed.system).")
        raise e
...
def dir_exists(dirpath):
    try:
        ...
        if DEBUG_FLAG:
            sys.stderr.write(
                "Naked Framework Error: error with test for directory with the dir_exists() function (Naked.toolshed.system).")
        raise e
...
def is_dir(dirpath):
    try:
        ...
        if DEBUG_FLAG:
            sys.stderr.write(
                "Naked Framework Error: error with test for directory with the is_dir() function (Naked.toolshed.system).")
        raise e
...
def file_size(filepath):
    try:
        ...
        if DEBUG_FLAG:
            sys.stderr.write(
                "Naked Framework Error: unable to return file size with the file_size() function (Naked.toolshed.system).")
        raise e
...
def file_mod_time(filepath):
    try:
        ...
        if DEBUG_FLAG:
            sys.stderr.write(
                "Naked Framework Error: unable to return file modification data with the file_mod_time() function (Naked.toolshed.system).")
        raise e
...
def list_all_files(dir):
    try:
        ...
        if DEBUG_FLAG:
            sys.stderr.write(
                "Naked Framework Error: unable to generate directory file list with the list_all_files() function (Naked.toolshed.system).")
        raise e
...
def list_filter_files(extension_filter, dir):
    try:
        ...
        if DEBUG_FLAG:
            sys.stderr.write(
                "Naked Framework Error: unable to return list of filtered files with the list_filter_files() function (Naked.toolshed.system).")
        raise e
...
def list_all_files_cwd(dir=""):
    try:
        ...
        if DEBUG_FLAG:
            sys.stderr.write(
                "Naked Framework Error: unable to return list of all files in current working directory with the list_all_files_cwd() function (Naked.toolshed.system).")
        raise e
...
def list_filter_files_cwd(extension_filter, current_dir=""):
    try:
        ...
        if DEBUG_FLAG:
            sys.stderr.write(
                "Naked Framework Error: unable to return list of filtered files in current working directory with the list_filter_files_cwd() function (Naked.toolshed.system).")
        raise e
...
def list_match_files(match_pattern, full_path=False):
    try:
        ...
        if DEBUG_FLAG:
            sys.stderr.write(
                "Naked Framework Error: unable to return list of matched files with the list_match_files() function (Naked.toolshed.system).")
        raise e
...
def is_link(filepath):
    try:
        ...
        if DEBUG_FLAG:
            sys.stderr.write(
                "Naked Framework Error: unable to determine whether path is a symbolic link with the is_link() function (Naked.toolshed.system).")
        raise e
...
def real_path(filepath):
    try:
        ...
        if DEBUG_FLAG:
            sys.stderr.write(
                "Naked Framework Error: unable to return real path for symbolic link with the real_path() function (Naked.toolshed.system).")
        raise e
...
def stdout(text):
    try:
        ...
        if DEBUG_FLAG:
            sys.stderr.write(
                "Naked Framework Error: unable to print to the standard output stream with the stdout() function (Naked.toolshed.system).")
        raise e
...
def stdout_xnl(text):
    try:
        ...
        if DEBUG_FLAG:
            sys.stderr.write(
                "Naked Framework Error: unable to print to the standard output stream with the stdout_xnl() function (Naked.toolshed.system).")
        raise e
...
def stdout_iter(iter):
    try:
        ...
        if DEBUG_FLAG:
            sys.stderr.write(
                "Naked Framework Error: unable to print to the standard output stream with the stdout_iter() function (Naked.toolshed.system).")
        raise e
...
def stderr(text, exit=0):
    try:
        ...
        if DEBUG_FLAG:
            sys.stderr.write(
                "Naked Framework Error: unable to print to the standard error stream with the stderr() function (Naked.toolshed.system).")
        raise e
