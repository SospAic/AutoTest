...
class XDict(dict, NakedObject):
    ...
    def __add__(self, other_dict):
        try:
            ...
            if DEBUG_FLAG:
                sys.stderr.write(
                    "Naked Framework Error: unable to combine XDict with parameter provided (Naked.toolshed.types.py)")
            raise e
...
class XList(list, NakedObject):
    ...
    def __add__(self, *other_lists):
        try:
            ...
            if DEBUG_FLAG:
                sys.stderr.write(
                    "Naked Framework Error: unable to combine XList with parameter provided (Naked.toolshed.types.py)")
            raise e
...
class XFSet(frozenset):
    def __new__(cls, the_set, attributes={}, naked_type="XFSet"):
        ...
        setattr(set_obj, '_naked_type_',
                naked_type)  # set the naked extension type as an attribute (NakedObject does this for mutable classes)
        return set_obj
